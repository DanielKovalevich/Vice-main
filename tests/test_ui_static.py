...
class UIStaticCopyTests(unittest.TestCase):
    ...
    def test_fireshare_publish_workflow_is_wired(self) -> None:
        ...
        self.assertIn("fireShareResponseJson", fireshare_js)
        self.assertNotIn("await r.json()", fireshare_js)
        self.assertIn("const message = err?.message || 'Validation failed'", fireshare_js)
        self.assertIn("applyFireShareAttempt(slug, d.attempt)", fireshare_js)
        self.assertIn("current?.cancelable === false", fireshare_js)
        self.assertIn("msg.type === 'fireshare_publish_progress'", fireshare_js)
        self.assertIn("msg.attempt_id !== current.attempt_id", fireshare_js)
